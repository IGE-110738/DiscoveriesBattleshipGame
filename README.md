# Discoveries Battleship Game

**Grupo:** [Brainrotters]  
**Curso:** IGE / IGE-PL (2025/2026)

## Membros

| Nº | Nome | GitHub |
|---:|------|--------|
| [110738] | [Nicholas Lobato] | IGE-[110738] |
| [XXXXX] | [Nome Apelido] | IGE-[XXXXX] |
| [XXXXX] | [Nome Apelido] | IGE-[XXXXX] |
| [XXXXX] | [Nome Apelido] | IGE-[XXXXX] |

## Navios (versão Descobrimentos)

| Batalha Naval | Descobrimentos (PT) | English | Dimensão | Nº de navios |
|---|---|---|---:|---:|
| Porta-aviões | Galeão | Galleon | 5 | 1 |
| Navio de 4 canhões | Fragata | Frigate | 4 | 1 |
| Navio de 3 canhões | Nau | Carrack | 3 | 2 |
| Navio de 2 canhões | Caravela | Caravel | 2 | 3 |
| Submarino | Barca | Barge | 1 | 4 |

## Regras do jogo (resumo)

1. Cada jogador tem 2 grelhas 10x10: a sua frota e o registo dos tiros no adversário.
2. Os navios são colocados na grelha na horizontal ou vertical.
3. Os navios não podem tocar-se entre si.
4. Em cada turno, o jogador atira **3 tiros**, indicando coordenadas (linha, coluna).
5. O adversário responde com o resultado (água / acerto / navio afundado e tipo).
6. Ganha quem afundar primeiro todos os navios do adversário.
